...
import (
	...
	"github.com/yrdsm666/tockowl/consensus/sdumbo/pb"
...
func MainProcess(p *party.HonestParty, ID []byte, input []byte) [][]byte {
	...
	resultValue := smvba.MainProcess(ctx, p, ID, value, validation, Q)
	cancel()
...
func Q(p *party.HonestParty, ID []byte, value []byte, validation []byte, hashVerifyMap *sync.Map, sigVerifyMap *sync.Map) error {
	...
	for i := uint32(0); i < p.N-p.F; i++ {
		...
		buf.Write(L.Hash[i])
		sm := buf.Bytes()
		err := crypto.VerifySignature(p.Config.Keys.ThresholdPK, sm, S.Sig[i]) //verify("Echo"||e||j||h)
		if err != nil {
			return err
		}
