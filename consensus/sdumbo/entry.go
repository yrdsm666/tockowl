...
import (
	...
	"github.com/yrdsm666/tockowl/consensus/sdumbo/aab"
...
func sdumboMvbaTest(p *party.HonestParty) {
	...
	for k := uint32(0); k < uint32(epoch); k++ {
		...
		res := smvba.MainProcess(ctx, p, ID, tx, nil, Q)
		cancel()
		...
		fmt.Printf("[SPEEDING-DUMBO MVBA] Epoch %d: GOOD WORK for replica %d: %s\n", k, p.ID, hex.EncodeToString(res)[:10])
	}
}
