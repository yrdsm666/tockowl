...
import (
	...
	"github.com/yrdsm666/tockowl/consensus/sdumbo/acs"
...
func SdumboAcsTest(p *party.HonestParty, outputChannel chan []byte) {
	...
	for e := uint32(0); e < uint32(epoch); e++ {
		...
		fmt.Printf("[SPEEDING-DUMBO ACS] Epoch %d: GOOD WORK for replica %d: %s\n", e, p.ID, hex.EncodeToString(output)[:10])
	}
}
