...
func OptPath1(ctx context.Context, p *party.HonestParty, ID []byte, input []byte, leader uint32, candidateChannel chan *PathOutput, commitChannel chan *PathOutput) {
	...
	for {
		...
		case m := <-p.GetMessage("Finish", ID):
			...
			buf.Write(h[:])
			sm := buf.Bytes()
			// p.Log.Info("[p=%s]: Commit a value in OptPath1", string(payload.Sig))
